            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-4">
                <form action="{{ route('feeType.update', $feeType->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input name="amount" id="name" type="number" min="0.0" step="0.00" value="{{ $feeType->amount }}" placeholder="Total Fee" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        @error('amount')
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
                    </div>
